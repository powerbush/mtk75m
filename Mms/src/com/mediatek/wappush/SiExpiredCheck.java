...
public class SiExpiredCheck {
                ...
                if (cursor.moveToFirst()) {
                    do{
                        ...
                        if(expiredTime > 0 && expiredTime < currentTime && isExpired == 0 ){
                        	Xlog.i(TAG,"SiExpiredCheck: message " + messageId + " is expired!");
                        ...
                        if(expiredTime > currentTime && isExpired == 1 ){
                        	Xlog.i(TAG,"SiExpiredCheck: message " + messageId + " is set noexpired!");
	...
	public void startSiExpiredCheckThread(){
		try{
			...
			Xlog.w(TAG,"SiExpiredCheck: thread start error!");
		}
	}
	...
	public void stopSiExpiredCheckThread(){
		try{
			...
			Xlog.w(TAG,"SiExpiredCheck: thread stop error!");
		}
	}
}
