...
public class HttpUtils {
    ...
    protected static byte[] httpConnection(Context context, long token,
        ...
        Xlog.d(MmsApp.TXN_TAG, "httpConnection: params list");
        Xlog.d(MmsApp.TXN_TAG, "\ttoken\t\t= " + token);
        Xlog.d(MmsApp.TXN_TAG, "\turl\t\t= " + url);
        Xlog.d(MmsApp.TXN_TAG, "\tmethod\t\t= "
                    + ((method == HTTP_POST_METHOD) ? "POST"
                            : ((method == HTTP_GET_METHOD) ? "GET" : "UNKNOWN")));
        Xlog.d(MmsApp.TXN_TAG, "\tisProxySet\t= " + isProxySet);
        Xlog.d(MmsApp.TXN_TAG, "\tproxyHost\t= " + proxyHost);
        Xlog.d(MmsApp.TXN_TAG, "\tproxyPort\t= " + proxyPort);
        ...
        try {
            ...
            try {
                Xlog.d(MmsApp.TXN_TAG, "===before execute ");
                response = client.execute(target, req);
                Xlog.d(MmsApp.TXN_TAG, "===after execute, response is null = " 
                        + ((null == response) ? true : false));
            } catch (IOException e){
                e.printStackTrace();
                Xlog.e(MmsApp.TXN_TAG, "AndroidHttpClient.execute exception: " + e.getMessage());
            }

            //HttpResponse response = client.execute(target, req);
            StatusLine status = response.getStatusLine();
            Xlog.d(MmsApp.TXN_TAG, "httpConnection: execute status="+status);
    ...
    private static void handleHttpConnectionException(Exception exception, String url)
        ...
        Xlog.e(MmsApp.TXN_TAG, "handle Http Connection Exception, Url: " + url + "\n" + exception.getMessage());
