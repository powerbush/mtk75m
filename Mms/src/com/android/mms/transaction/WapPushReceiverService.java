...
//        }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        // Temporarily removed for this duplicate message track down.
//        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
//            Xlog.v(TAG, "onStart: #" + startId + ": " + intent.getExtras());
//        }
...
//        }
    ...
    private void handleWapPushReceived(Intent intent, int error) {
        ...
        Xlog.i(TAG, "handleWapPushReceived: " + intent.getAction());
        ...
        Xlog.i(TAG, "handleWapPushReceived: " + sender + service_center + " " + simId);
        if(sender == null || service_center == null){
        	Xlog.e(TAG, "handleWapPushReceived: sender or service center is null!");
        	return;
        }
        ...
        if(parser!=null){
            ...
            Xlog.e(TAG,"Wap Push message parse create error!");
        }
        
        if(pushMsg != null){
            Xlog.i(TAG,pushMsg.toString());
            manager = WapPushManager.createManager(this,pushMsg.type());
        }else{
        	Xlog.e(TAG,"Wap Push Message parseData error!");
        }
...
//            Xlog.v(TAG, "handleSmsReceived" + (sms.isReplace() ? "(replace)" : "") +
...
//        }
    ...
    private void registerForServiceStateChanges() {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Xlog.v(TAG, "registerForServiceStateChanges");
        }
    ...
    private void unRegisterForServiceStateChanges() {
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Xlog.v(TAG, "unRegisterForServiceStateChanges");
        }
        try {
            Context context = getApplicationContext();
