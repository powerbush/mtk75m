        ...
        private class Row {
            ...
            public String getWord() {
				...
				try{
					...
					Xlog.w(LOG_TAG, "UnsupportedEncodingException");
				}
				...
				try {
					...
					Xlog.e(LOG_TAG, "IndexOutOfBoundsException");
        ...
        private void computeRows() {
            ...
            for (int i = 0; i < count; i++) {
                ...
                for (int j = 0; j < offsets.length; j += 4) {
                    /**
                     *columnNumber The column number that the term instance occurs in .  
                     *termNumber  The term number of the matching term within the full-text query expression.  
                     *startOffset  The byte offset of the matching term within the column.  
                     *length  The size of the matching term in bytes.  
                     */
                    int columnNumber = offsets[j+0];
                    if (textColumn == columnNumber) {
//                      int termNumber   = offsets[j+1];
                        ...
                        String candidate = null;
                        byte[] btarray = null;
                        Xlog.d(LOG_TAG,"computeRows. message="+message+". messge length: "+message.length() + "; startOffset="+startOffset+"; endOffset"+endOffset);
                        try {
                           btarray = message.getBytes(CHARSET_NAME);
                        } catch (UnsupportedEncodingException ex) {
                           Xlog.e(LOG_TAG, "UnsupportedEncodingException");
                        }
                        ...
                        Xlog.d(LOG_TAG, "btarray=" + btarray + "; result =" + candidate);
                        String key = null;
                        if (candidate != null) {
