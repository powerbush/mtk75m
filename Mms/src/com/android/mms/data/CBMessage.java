...
public class CBMessage {
    ...
    private static final Uri MESSAGE_URI = Telephony.CbSms.CONTENT_URI;// Uri.parse("content://cb/messages");
    // Query all URI
    private static final Uri sAllMessageUri = Telephony.CbSms.CONTENT_URI;
    // Uri.parse("content://cb/messages");
    // Query messge URI by thread_id
    public static final Uri sThreadMessageUri = Uri
            .parse("content://cb/messages/#");

    private static final Uri CHANNEL_ID_URI = Uri
            .parse("content://cb/channel/#");

    private static final String[] ALL_MESSAGE_PROJECTION = { "_id",
            Telephony.CbSms.SIM_ID, Telephony.CbSms.CHANNEL_ID,
            Telephony.CbSms.BODY, Telephony.CbSms.READ,
            Telephony.CbSms.DATE, Telephony.CbSms.THREAD_ID };
    ...
    // The thread ID of this CBMessages. Can be zero in the case of a
    // new CBMessages where the recipient set is changing as the user
    ...
    private String mBody; // Text of the most recent message.
    ...
    private static boolean mLoadingMessages;
    ...
    private synchronized void setChannelNameFromDatabase() {
        Uri uri = ContentUris.withAppendedId(CHANNEL_ID_URI, mChannelId);
...
//        return String.format("[%s] (tid %d)", mRecipients.serialize(),
//                mThreadId);
//    }
    ...
    public static void startQueryForAll(AsyncQueryHandler handler, int token) {
        ...
        handler.startQuery(token, null, sAllMessageUri, ALL_MESSAGE_PROJECTION,
    ...
    private static void cacheAllMessages(Context context) {
        ...
        Cursor c = context.getContentResolver().query(sAllMessageUri,
                ALL_MESSAGE_PROJECTION, null, null, null);
        try {
            if (c != null) {
            	long messageId = 0L;
    ...
    private boolean loadFromId(long messageId, boolean allowQuery) {
        Cursor c = mContext.getContentResolver().query(sAllMessageUri,
