...
public class VTSettingUtils{
	...
	public boolean mPeerBiggerLastValue;
	...
	public boolean mToReplacePeerLastValue;
	...
	public void resetVTSettingToDefaultValue()
	{
		...
		mPeerBigger = true;
		mPeerBiggerLastValue = true;
		mShowLocalMO = true;
		mShowLocalMT = "0";
		
		mAutoDropBack = false;
		mToReplacePeer = true;
		mToReplacePeerLastValue = true;
		mPicToReplacePeer = "0";
	}
    ...
    public void updateVTSettingState(){
    	...
    	try{
    		...
    		mEnableBackCamera = sp.getBoolean("button_vt_enable_back_camera_key", true);
                mPeerBiggerLastValue = mPeerBigger;
    		...
    		mAutoDropBack = sp.getBoolean("button_vt_auto_dropback_key", false);
                mToReplacePeerLastValue = mToReplacePeer;
        ...
        if (DBG) log(" - mPeerBiggerLastValue = " + mPeerBiggerLastValue);
        if (DBG) log(" - mToReplacePeerLastValue = " + mToReplacePeerLastValue);
		...
		if (mPicToReplacePeer.equals(VTAdvancedSetting.SELECT_DEFAULT_PICTURE2)) {
			if (mToReplacePeer)
				VTManager.getInstance().replacePeerVideoSettings(1,
						VTAdvancedSetting.getPicPathDefault2());
			else
				VTManager.getInstance().replacePeerVideoSettings(0,
						VTAdvancedSetting.getPicPathDefault2());
		} else {
			if (mToReplacePeer)
				VTManager.getInstance().replacePeerVideoSettings(1,
